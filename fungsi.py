# Line 2-8 = Import module yang diperlukan
from PyQt5.QtWidgets import QGridLayout, QLabel, QPushButton
from PyQt5.QtGui import QPixmap, QCursor
from PyQt5 import QtCore
from urllib.request import urlopen #Agar bisa masuk ke url APInya
import json #Databasenya dalam format json
import pandas as pd
import random

# Line 11-14 = Buka API Link ke database
with urlopen("https://opentdb.com/api.php?amount=50&category=18&difficulty=medium&type=multiple") as webpage:
    # Line 13-14 = Read file JSON & ekstrak data
    data = json.loads(webpage.read().decode())
    df = pd.DataFrame(data["results"])

# Line 17-53 = load 1 instance dari pertanyaan & jawaban sekaligus dari database
def preload_data(idx):
    #idx parmeter = dipilih secara acak berkali-kali pada function call
    question = df["question"][idx]
    correct = df["correct_answer"][idx]
    wrong = df["incorrect_answers"][idx]

    # Line 24-30 = Memperbaiki karakter yang formatnya buruk
    formatting = [
        ("#039;", "'"),
        ("&'", "'"),
        ("&quot;", '"'),
